...
from scrubboard.redact import Entity, normalize_label, placeholder_for

# ---------------------------------------------------------------------------
# Constants: tag groups, label mappings, keep-lists, eponyms
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# SafeHarborPolicy: filters entities and renders Safe Harbor transforms
# ---------------------------------------------------------------------------
...
_TEENS = {"thirteen": 13, "fourteen": 14, "fifteen": 15, "sixteen": 16,
...
def _age_value(span: str) -> int | None:
    ...
    if "hundred" in low:
        return 100
    for word, value in _TEENS.items():
        if word in low:
            return value
...
def _born_year_ok(val: str, _m: re.Match, _text: str) -> bool:
    ...
    try:
        ...
        return True  # keep the text if we can't parse the year (fail-safe)
