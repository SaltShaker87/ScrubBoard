"""Persistent config: ~/.scrubboard/config.json"""
...
_BOOL_FIELDS = ("intercept_enabled", "keep_year", "keep_zip3", "llm_enabled", "onboarding_done")
_LIST_FIELDS = ("exclude_apps", "extra_keep_labels")


def config_dir() -> str:
    return os.path.join(os.path.expanduser("~"), ".scrubboard")
...
@dataclass
class ScrubboardConfig:
    ner_model: str = "openmed-44m"
    threshold: float | None = None  # None = model default from the manifest
    intercept_enabled: bool = False  # off = clean only on request (tray / macOS Services)
    ...
    onboarding_done: bool = False  # welcome + disclaimer shown (installers set this)
    ...
    @classmethod
    def load(cls, path: str | None = None) -> ScrubboardConfig:
...
def coerce_value(key: str, raw: str):
    ...
    ftypes = {f.name: str(f.type) for f in fields(ScrubboardConfig) if f.name != "load_warning"}
