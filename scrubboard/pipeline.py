...
from scrubboard.redact import Entity, expand_to_word_boundaries, redact_text, resolve_overlaps, truncate_text
from scrubboard.safe_harbor import SafeHarborPolicy, SafeHarborRules

TRUNCATION_MARKER = "\n[Scrubboard: truncated at {n} characters]"
...
class RedactionPipeline:
    def __init__(self, config: ScrubboardConfig, ner: PIIModel | None = None,
    ...
    def ensure_ready(self) -> None:
        ...
        if self.config.ner_model == "none" or self._ner_loaded:
            return
        from scrubboard.models import load_ner
        try:
    ...
    def set_llm(self, llm: LLMEngine | None) -> None:
        """Thread-safe setter for the LLM engine (called from app thread)."""
        with self._lock:
            self.llm = llm
