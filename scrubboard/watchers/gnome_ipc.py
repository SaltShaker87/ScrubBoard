...
def socket_path() -> str:
    return os.path.join(os.environ.get("XDG_RUNTIME_DIR") or tempfile.gettempdir(), "scrubboard.sock")
...
class GnomeIPCServer:
    ...
    def start(self) -> None:
        ...
        sock.listen(8)
        self._sock = sock
        threading.Thread(target=self._serve, name="scrubboard-gnome-ipc", daemon=True).start()
