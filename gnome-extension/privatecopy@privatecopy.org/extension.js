// PrivateCopy GNOME Shell extension (GNOME 45+).
//
// GNOME on Wayland lets only the compositor watch the clipboard, so this
// extension watches it and asks the local PrivateCopy daemon (over a 0600 Unix
...
const OWN_PREFIX = '[PrivateCopy:';
const PASSWORD_HINT = 'x-kde-passwordManagerHint';

export default class PrivateCopyExtension extends Extension {
    enable() {
    ...
    _redact(text, app, generation) {
        const path = GLib.build_filenamev([GLib.get_user_runtime_dir(), 'privatecopy.sock']);
