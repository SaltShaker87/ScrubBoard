...
def create_backend(name: str | None = None) -> ClipboardBackend:
    ...
    if name == "macos":
        from privatecopy.watchers.macos import MacClipboard
        return MacClipboard()
    if name == "windows":
        from privatecopy.watchers.windows import WindowsClipboard
        return WindowsClipboard()
    if name == "x11":
        from privatecopy.watchers.x11 import X11Clipboard
        return X11Clipboard()
    if name in ("wlclipboard", "gnome"):
        from privatecopy.watchers.wlclipboard import WlClipboard
