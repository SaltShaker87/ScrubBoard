...
from privatecopy.llm.engine import LLMEngine


def create_llm_engine(config: PrivateCopyConfig, warm: bool = True) -> LLMEngine:
    ...
    from privatecopy.download import llm_model_path
    from privatecopy.llm.runtime import LlamaServer, find_llama_server
    ...
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"{entry['file']} is not downloaded. "
                                f"Run: privatecopy download-model --llm {config.llm_model}")
    binary = find_llama_server(config.llm_server_path)
    if not binary:
        raise RuntimeError("llama-server not found. Reinstall PrivateCopy, or run: "
