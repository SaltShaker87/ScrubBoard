"""NVIDIA GLiNER-PII via the optional `gliner` package (pip install 'privatecopy[gliner]')."""
...
class GlinerModel(PIIModel):
    ...
    def load(self) -> None:
        ...
        try:
            ...
            raise RuntimeError("nvidia-gliner needs the gliner package: pip install 'privatecopy[gliner]'") from e
        if not (os.path.isdir(self.model_dir) and os.listdir(self.model_dir)):
            raise FileNotFoundError(f"GLiNER weights not found in {self.model_dir}. "
                                    "Run: privatecopy download-model --ner nvidia-gliner")
