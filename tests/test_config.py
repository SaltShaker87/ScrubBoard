...
from privatecopy.config import PrivateCopyConfig, coerce_value
...
def test_defaults():
    ...
    assert cfg.intercept_enabled and not cfg.llm_enabled
    assert cfg.hotkey == ""
    cfg.validate()
...
def test_save_load_roundtrip(tmp_path, monkeypatch):
    ...
    cfg = PrivateCopyConfig(ner_model="nvidia-gliner", threshold=0.7, hotkey="ctrl+shift+p",
                            exclude_apps=["Epic.exe"])
    loaded = PrivateCopyConfig.load(cfg.save())
...
def test_invalid_config_falls_back_to_defaults(tmp_path):
    ...
    cfg = PrivateCopyConfig.load(str(p))
    assert cfg.ner_model == "openmed-44m"
...
def test_corrupt_json_falls_back(tmp_path):
    ...
    assert PrivateCopyConfig.load(str(p)).load_warning
